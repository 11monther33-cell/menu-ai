/**
 * useThreeEngine.ts — TableX Core 3D Engine
...
export function useThreeEngine(opts: UseThreeEngineOptions): ThreeEngineAPI {
  ...
  useEffect(() => {
    ...
    loader.load(
      ...
      err => {
        console.error('[TableX 3D] Model load error:', err)
        onError('فشل تحميل النموذج. تأكد من صيغة الملف GLB.')
      }
    )
    ...
    const animate = () => {
      ...
      if (autoRotRef.current && modelRef.current) {
  ...
  return { setMode, captureFrame, resetCamera, setAutoRotate, getPolyCount }
}
