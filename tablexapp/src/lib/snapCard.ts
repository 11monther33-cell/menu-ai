/**
 * snapCard.ts — TableX Snap Card Generator
...
export async function buildSnapCard(opts: SnapCardOptions): Promise<Blob> {
  const { dishFrame, dishName, price, primaryColor, streakCount, restaurantName } = opts
  ...
  ctx.textAlign    = 'center'
  ctx.textBaseline = 'middle'
  ctx.fillText('TableX', W - 90, 48)

  // ── Dish name ─────────────────────────────────────────────────────
  ctx.fillStyle    = 'white'
  ctx.font         = `bold 88px Cairo, Arial`
  ctx.textAlign    = 'center'
  ...
  ctx.font         = '26px Space Grotesk, Arial'
  ctx.fillText('tablexapp.com', W / 2, H - 40)
...
// Helper: rounded rect (Canvas doesn't have this natively in older browsers)
function roundRect(
