...
export const Footer = () => {
  ...
  return (
    <footer className="bg-[#351344] text-white pt-24 pb-12 relative overflow-hidden">
      <div className="site-noise"></div>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* Final CTA */}
        <div className="relative p-12 md:p-20 rounded-2xl bg-[#260D32] border border-white/10 overflow-hidden mb-24 text-center">
          ...
          <div className="relative z-10">
            <h2 className="text-4xl md:text-5xl font-display mb-6 leading-tight max-w-3xl mx-auto text-white">
              {isRtl ? 'حوّل مطعمك لتجربة لا تُنسى' : 'Transform Your Restaurant Into An Unforgettable Experience'}
            </h2>
            <p className="text-[#E8DDED] text-lg mb-12 max-w-2xl mx-auto">
              {isRtl 
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              <button className="w-full sm:w-auto bg-[#FF5A1F] hover:bg-[#C4400D] text-white font-semibold px-8 py-4 rounded-lg text-sm transition-colors uppercase tracking-wider shadow-lg">
                {t('hero.ctaPrimary')}
              </button>
              <button className="w-full sm:w-auto bg-[#25D366] hover:bg-[#1DA851] text-white font-semibold px-8 py-4 rounded-lg text-sm flex items-center justify-center gap-2 transition-colors uppercase tracking-wider shadow-lg">
                <MessageCircle size={18} />
                {isRtl ? 'واتساب مباشر' : 'Direct WhatsApp'}
              </button>
            </div>

            <div className="mt-12 flex flex-wrap justify-center gap-8 text-[#E8DDED]/80 text-xs uppercase tracking-widest font-medium">
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-12 mb-16">
          ...
          {columns.map((col, i) => (
            <div key={`footer-col-${i}-${col.title}`}>
              <h4 className="font-semibold mb-6 text-white text-sm uppercase tracking-widest">{col.title}</h4>
              <ul className="space-y-4">
                {col.links.map((link, j) => (
                  <li key={`footer-link-${i}-${j}-${link.label}`}>
                    <a href={link.href} className="text-[#E8DDED] hover:text-[#FF5A1F] transition-colors text-sm">{link.label}</a>
                  </li>
                ))}
              </ul>
            </div>
        ...
        <div className="pt-8 border-t border-white/10 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-[#E8DDED]/60 text-xs">{t('footer.rights')}</p>
          <div className="flex gap-6">
            <a href="#" className="text-[#E8DDED] hover:text-[#FF5A1F] transition-colors text-sm">Instagram</a>
            <a href="#" className="text-[#E8DDED] hover:text-[#FF5A1F] transition-colors text-sm">Twitter</a>
            <a href="#" className="text-[#E8DDED] hover:text-[#FF5A1F] transition-colors text-sm">LinkedIn</a>
          </div>
        </div>
      </div>
    </footer>
